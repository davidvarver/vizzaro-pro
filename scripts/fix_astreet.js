...
const TARGET_NAME = 'A-Street Select - 4021';
...
async function main() {
    ...
    try {
        ...
        // 2. Find Folder
        let rootPath = '/WallpaperBooks/A-Street Select - 4021';
        if (!await sftp.exists(rootPath)) {
            console.log(`⚠️ Path ${rootPath} not found. Searching...`);
            const books = await sftp.list('/WallpaperBooks');
            const match = books.find(b => b.name.includes('A-Street') || b.name.includes('Street'));
            if (match) {
